...
func validatePermission(c echo.Context, permission string) error {
	...
	if claims, ok := token.Claims.(jwt.MapClaims); ok {
		...
		nlogs.NLog("warning", "validatePermission", map[string]interface{}{"message": fmt.Sprintf("user dont have permission %v", permission)}, c.Get("user").(*jwt.Token), "", false)

		return fmt.Errorf("Tidak memiliki hak akses")
	}

	nlogs.NLog("warning", "validatePermission", map[string]interface{}{"message": "invalid token. error claims"}, c.Get("user").(*jwt.Token), "", true)
